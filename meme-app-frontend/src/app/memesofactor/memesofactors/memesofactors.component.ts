...
import { MemecommonService } from 'src/app/memecommon.service';
import { NavbarService } from 'src/app/navbar.service';

@Component({
  selector: 'app-memesofactors',
  templateUrl: './memesofactors.component.html',
  styleUrls: ['./memesofactors.component.css']
})
export class MemesofactorsComponent implements OnInit {
 ...
 actorName:any;
  ...
  constructor(private backend:BackendService,private sanitizer: DomSanitizer,
    private navbar:NavbarService,private nav:NavbarService, private memeService:MemecommonService,
    private router:Router) { }

  ngOnInit(): void {
    this.showOnlyTheResultsOfTheActor();
  }
...
downloadMeme(dataurl:any, filename:any){
   ...
   showOnlyTheResultsOfTheActor(){
    this.actorName = this.memeService.getActor();
    this.backend.getImageByActorName(this.actorName).subscribe((res:any)=>{
      this.getAllImages = res;
      this.createBlobImageFileAndShow();
      this.actorNameToArray(this.actorName);
     });
   }
